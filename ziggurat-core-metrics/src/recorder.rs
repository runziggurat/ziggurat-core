...
#[cfg(test)]
mod tests {
    use metrics::{histogram, register_counter, register_gauge, register_histogram};
    ...
    fn can_construct_histogram() {
        let metrics = TestMetrics::default();
        register_histogram!(METRIC_NAME);

        histogram!(METRIC_NAME, 1.0);
        histogram!(METRIC_NAME, 3.0);
        histogram!(METRIC_NAME, 5.0);
        histogram!(METRIC_NAME, 9.0);
