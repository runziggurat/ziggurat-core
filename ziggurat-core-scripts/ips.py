...
# Sample invocation:
# add whole 9.1.1.0/24 subnet creating dummy devices test_zeth0...test_zeth248 and resulting IPs to file: tools/ips_list.json
# python3 ./tools/ips.py --subnet 9.1.1.0/24 --file tools/ips_list.json --dev_prefix test_zeth
#
# remove all test_zeth* devices from 24 subnet (clear tools/ips_list.json file)
# python3 ./tools/ips.py --subnet 9.1.1.0/24 --file tools/ips_list.json --dev_prefix test_zeth --op remove
#
# add whole 9.1.1.0/24 subnet to device lo (for Mac use lo0) and resulting IPs to file: tools/ips_list.json
# python3 ./tools/ips.py --subnet 9.1.1.0/24 --file tools/ips_list.json --dev lo
#
# remove whole 9.1.1.0/24 subnet from device lo (clear tools/ips_list.json file)
# python3 ./tools/ips.py --subnet 9.1.1.0/24 --file tools/ips_list.json --dev lo --op remove
...
parser.add_argument('--file', nargs='?', default='ips_list.json', help='Output file with IPs (default ips_list.json)', type=str)
    ...
    for ip in ips:
        ...
        if res == 0 and op == 'add':
            if i != len(ips) - 1:
                ...
                # json has no comma for last item in array
                f.write('\t"' + str(ip) + '"\n')
        i += 1
